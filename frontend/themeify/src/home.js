...
function Home() {
    ...
    useEffect(() => {
        // const wordContainer = document.createElement("word-  container");
        // wordContainer.class = "words"
        ...
        api.post('/access-token-grant', { headers: header }, { params : {
            code : code 
          }
        })
        .then(res => {
            setLoadStatus('getting top tracks...')
            api.get('/user-top-tracks', { params : {
            ...
            .then(res => {
                if (res.data.userExists) {
                    ...
                    setThemes(res.data.existingThemes)
                    setLoadStatus(false)
                    return;
                }  
                setUserInfo({'displayName' : res.data.user.display_name, 'userID' : res.data.user.id, 'imageURL' : res.data.user.images[0].url})
                setLoadStatus('discovering ' + res.data.user.display_name + '\'s vibes...')
                // Check if different from last load before calling new theme extraction
                api.get('/find-themes', { params : {
                    userID : res.data.user.id,
                    lyrics : JSON.stringify(res.data.lyrics)
                }})
                .then(res => {
    ...
    useEffect(() => {
        if (themes != null) { 
            ...
            let wordContainer = document.getElementById('themesGraphic')
            for (let i = 0; i < themes.length; i++) {
                let currTheme = themes[i]
                let currPos = 'pos' + (i+1).toString()
                console.log('pos is ', currPos)
    ...
    return (
        <>
            ...
            <>
                <div id='themesGraphic' className='text-white font-display'>
                    <img src={singleBlob}></img>
                    <div id="graphicTitle" className="text-m" style={{fontSize:1.5+'vw'}}>
